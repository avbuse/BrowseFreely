...
- 🚫 **Anti-adblock bypass**: Extra Fanboy/uBO annoyance lists, cosmetic + HTML filters, silent 200 stubs, detection stubs, and for Future plc / Ad-Shield sites (Windows Central, …) **strip detection loaders** + disable `"adShield"` in page config (tinyShield only as fallback).
...
* `DISABLE_TINYSHIELD` - `true` to never inject tinyShield (default: inject only as fallback when Ad-Shield config flip fails)
* `FORCE_TINYSHIELD` - `true` to always inject tinyShield on Future/Ad-Shield hosts
